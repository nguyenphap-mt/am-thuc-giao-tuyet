...
export function WeekView({ currentDate, events, isLoading, onEventClick, onDateClick }: WeekViewProps) {
 ...
 ${isToday ? 'bg-accent-50' : 'bg-gray-50 dark:bg-gray-900'}
 `}
 >
 <p className="text-xs text-gray-500 dark:text-gray-400">{DAYS_OF_WEEK_FULL[date.getDay()]}</p>
 <p className={`text-lg font-bold tabular-nums ${isToday ? 'text-accent-primary' : 'text-gray-900 dark:text-gray-100'}`}>
 {date.getDate()}
 </p>
 ...
 <div
 key={i}
 className={`border-r border-gray-100 dark:border-gray-800 p-0.5 relative ${isToday ? 'bg-accent-50' : ''}`}
 >

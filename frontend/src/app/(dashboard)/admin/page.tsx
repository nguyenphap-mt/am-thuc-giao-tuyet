...
const ADMIN_TABS = ['users', 'roles'] as const;

export default function AdminPage() {
 const { activeTab, handleTabChange } = useTabPersistence(ADMIN_TABS, 'users');
 ...
 // Stats data
 const stats = [
 { label: 'Tổng người dùng', value: total, icon: IconUsers, color: 'text-accent-primary', bg: 'bg-accent-50' },
 ...
 const roles = [
 { name: 'Super Admin', count: 1, permissions: 'Toàn quyền hệ thống', color: 'bg-accent-solid' },
 ...
 >
 <div>
 <h1 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-gray-100">Quản trị hệ thống</h1>
 <p className="text-sm text-gray-500 dark:text-gray-400">Quản lý người dùng và phân quyền</p>
 </div>
 <Button
 onClick={() => refetch()}
 variant="outline"
 ...
 <TabsList className="w-full md:w-auto grid grid-cols-2 md:flex bg-gray-100 dark:bg-gray-800/80 p-1 rounded-lg">
 ...
 className="pl-10 h-10 focus-visible:ring-accent"
 />
 </div>
 ...
 initial={{ opacity: 0, y: -10 }}
 animate={{ opacity: 1, y: 0 }}
 className="flex items-center gap-3 p-3 bg-accent-50 rounded-lg border border-accent-subtle"
 >
 ...
 <IconUsers className="mx-auto h-12 w-12 text-gray-300 dark:text-gray-600 mb-4" />
 ...
 >
 {/* Header Row */}
 <div className="hidden md:flex items-center gap-3 px-4 py-3 bg-gray-50 dark:bg-gray-900 dark:bg-gray-800 border-b text-sm font-medium text-gray-500 dark:text-gray-400">
 <Checkbox
 ...
 className={`flex items-center gap-3 px-4 py-3 border-b last:border-b-0 transition-colors cursor-pointer ${selectedIds.includes(user.id) ? 'bg-accent-50' :
 hoveredId === user.id ? 'bg-gray-50 dark:bg-gray-900' : ''
 }`}
 >
 <Checkbox
 ...
 <AvatarFallback className="bg-accent-gradient-br to-purple-500 text-white text-sm font-medium">
 ...
 : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700'
 }>
 ...
 <IconEdit className="h-4 w-4 text-gray-500 dark:text-gray-400" />
 </Button>
 <Button
 variant="ghost"
 size="icon"
 ...
 <div>
 <h3 className="font-medium text-gray-900 dark:text-gray-100">Danh sách vai trò</h3>
 <p className="text-sm text-gray-500 dark:text-gray-400">Quản lý vai trò và phân quyền trong hệ thống</p>
 </div>
 <Button className="bg-accent-gradient hover:opacity-90 transition-opacity">
 <IconPlus className="mr-2 h-4 w-4" />
 Thêm vai trò
 ...
 <IconChevronRight className="h-5 w-5 text-gray-400 dark:text-gray-500 group-hover:text-accent-primary transition-colors" />
 </div>
 </CardContent>
 </Card>
 </motion.div>

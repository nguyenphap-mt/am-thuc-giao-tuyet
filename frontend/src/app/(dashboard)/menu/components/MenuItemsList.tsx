...
export function MenuItemsList({
 ...
 <button onClick={() => setFilterCategoryId(null)} className={cn('px-2.5 py-1 rounded-full text-xs font-medium whitespace-nowrap transition-colors cursor-pointer', !filterCategoryId ? 'bg-accent-gradient text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200')}>Tất cả</button>
 {categories.map(cat => (
 <button key={cat.id} onClick={() => setFilterCategoryId(filterCategoryId === cat.id ? null : cat.id)} className={cn('px-2.5 py-1 rounded-full text-xs font-medium whitespace-nowrap transition-colors cursor-pointer', filterCategoryId === cat.id ? 'bg-accent-gradient text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200')}>{cat.name}</button>
 ))}
 </div>
 ...
 <Button variant="ghost" size="icon" className="h-7 w-7" onClick={e => { e.stopPropagation(); onOpenRecipe(item.id); }}><IconChefHat className="h-4 w-4 text-accent-primary" /></Button>

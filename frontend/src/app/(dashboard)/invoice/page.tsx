    ...
    return (
        <div className="space-y-4">
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
                <h1 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-gray-100">Hóa đơn</h1>
                <p className="text-sm text-gray-500 dark:text-gray-400">Quản lý hóa đơn</p>
            </motion.div>
                    ...
                    <div className="divide-y">
                        {isLoading ? (
                            ...
                            <div className="text-center py-16">
                                <IconReceipt className="mx-auto h-12 w-12 text-gray-300 dark:text-gray-600" />
                                <p className="mt-4 text-gray-500 dark:text-gray-400">Chưa có hóa đơn</p>
                            </div>
                        ) : (
                            invoices.map((inv: any) => (
                                <div
                                    key={inv.id}
                                    className={`flex items-center gap-2 md:gap-4 px-2 md:px-4 py-2 md:py-3 cursor-pointer transition-colors ${selectedIds.includes(inv.id) ? 'bg-blue-50' : 'hover:bg-gray-50 dark:bg-gray-900 dark:hover:bg-gray-800'}`}
                                ...
                                >
                                    ...
                                    <button onClick={(e) => toggleStar(inv.id, e)} className="p-1 hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700 rounded">
                                        {starredIds.includes(inv.id) ? <IconStarFilled className="h-4 w-4 text-amber-400" /> : <IconStar className="h-4 w-4 text-gray-400 dark:text-gray-500" />}
                                    </button>

                                    <div className="flex-1 min-w-0 flex items-center gap-2 md:gap-4">
                                        <div className="w-24 md:w-32 truncate">
                                            <span className="font-medium text-sm text-gray-900 dark:text-gray-100">{inv.customer_name}</span>
                                        </div>
                                        <div className="flex-1 min-w-0 flex items-center gap-2">
                                            ...
                                            <span className="text-sm text-gray-500 dark:text-gray-400 truncate hidden sm:inline">{inv.invoice_number}</span>
                                        </div>
                                        <div className="text-right shrink-0">
                                            <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{formatCurrency(inv.total_amount)}</span>
                                        </div>
                                        <div className="w-20 text-right shrink-0 hidden lg:block">
                                            <span className="text-xs text-gray-500 dark:text-gray-400">{formatDate(inv.created_at)}</span>
                                        </div>
                                    </div>

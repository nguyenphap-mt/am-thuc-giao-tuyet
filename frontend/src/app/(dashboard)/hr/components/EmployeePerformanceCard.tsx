...
export function EmployeePerformanceCard({ employeeId, employeeName, periodDays = 30 }: Props) {
 ...
 if (isLoading) {
 return (
 <div className="bg-accent-50 rounded-xl p-6 border border-accent-subtle">
 <div className="animate-pulse space-y-4">
 <div className="h-5 bg-accent-200 rounded w-1/3"></div>
 <div className="grid grid-cols-2 gap-4">
 {[1, 2, 3, 4].map(i => (
 <div key={i} className="h-16 bg-accent-100 rounded-lg"></div>
 ))}
 </div>
 </div>
 </div>
 ...
 <IconTrendingUp className="w-5 h-5 text-accent-primary" />
 ...
 <button
 onClick={() => refetch()}
 className="p-1.5 hover:bg-accent-100 rounded-lg transition-colors"
 title="Làm mới"
 >
 <IconRefresh className="w-4 h-4 text-accent-primary" />
 </button>
 </div>
 </div>
 ...
 {/* Total Hours */}
 <div className="bg-white/70 rounded-lg p-3.5 border border-accent-subtle/50">
 ...
 <div className="mt-4 bg-white/70 rounded-lg p-3.5 border border-accent-subtle/50">

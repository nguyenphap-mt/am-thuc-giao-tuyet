...
export default function EmployeeLeaveView() {
    ...
    const cancelMutation = useMutation({
        mutationFn: (requestId: string) =>
            api.put(`/hr/leave/self/my-requests/${requestId}/cancel`),
        onSuccess: () => {
    ...
    const getLeaveIcon = (code: string) => {
        switch (code) {
            ...
            default:
                return <IconUser className="h-5 w-5 text-accent-primary" />;
        }
    };
                        ...
                        <Card key={balance.leave_type_code} className="hover:shadow-md transition-shadow">
                            <CardContent className="p-4">
                                ...
                                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                                    <div
                                        className="bg-accent-gradient to-purple-500 h-2 rounded-full transition-all"
                            ...
                            {myRequests.map((req) => (
                                <div key={req.id} className="flex items-center gap-4 px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-900">
                                    ...
                                    <div className="p-2 rounded-lg bg-accent-50 shrink-0">
                                        <IconBeach className="h-5 w-5 text-accent-primary" />
                                    </div>
                                    ...
                                    <div className="w-28 flex items-center gap-2">
                                        ...
                                        {req.status === 'PENDING' && (
                                            <button
                                                onClick={(e) => { e.stopPropagation(); setCancelRequestId(req.id); }}
            ...
            <Dialog open={!!cancelRequestId} onOpenChange={(open) => !open && setCancelRequestId(null)}>
                <DialogContent className="sm:max-w-[400px]">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Bạn có chắc chắn muốn hủy đơn nghỉ phép này? Hành động này không thể hoàn tác.
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter className="gap-2">
                        <Button variant="outline" onClick={() => setCancelRequestId(null)}>Không</Button>
                        <Button
                            variant="destructive"

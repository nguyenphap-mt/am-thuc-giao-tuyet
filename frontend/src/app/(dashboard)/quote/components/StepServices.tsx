...
export function StepServices({ state }: Props) {
 ...
 <IconBuildingStore className="h-4 w-4 text-accent-primary" />
 Bàn ghế &amp; Trang trí
 </h4>
 ...
 <div className="border rounded-lg p-4">
 <h4 className="font-medium text-gray-900 dark:text-gray-100 mb-4 flex items-center gap-2">
 <IconUsers className="h-4 w-4 text-accent-primary" />
 Nhân viên phục vụ
 </h4>
 {staffItems.length === 0 ? (
 ...
 <span className="text-accent-primary">{formatCurrency(serviceTotal)}</span>
 </div>
 </div>
 </div>
 );

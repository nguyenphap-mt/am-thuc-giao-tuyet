...
export function StepMenuSelection({ state }: Props) {
 ...
 <IconToolsKitchen2 className="h-4 w-4 text-accent-primary" />
 {group.category.name} ({group.items.length})
 {selectedInCategory > 0 && (
 <span className="inline-flex items-center justify-center px-2 py-0.5 rounded-full text-xs font-medium bg-accent-100 text-accent-strong dark:bg-accent-solid dark:text-accent-muted">
 ...
 ? 'bg-accent-50 border-2 border-accent-medium'
 ...
 <span>Tổng/bàn:</span>
 <span className="text-accent-primary" style={{ fontVariantNumeric: 'tabular-nums' }}>{formatCurrency(menuTotal)}</span>
 </div>

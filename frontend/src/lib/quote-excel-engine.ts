...
export async function exportQuoteExcel(
 ...
 // Summary mode: single merged row
 ws.mergeCells(row, 1, row, 5);
 fillCell(ws, row, 1, `${data.table_count} bàn × ${data.menuTotalPerTable.toLocaleString('vi-VN')} ₫/bàn = ${data.menuTotalWithTables.toLocaleString('vi-VN')} ₫`, {
 ...
 // Column headers without price
 const menuHeaders = ['STT', 'Tên món', '', '', ''];
 menuHeaders.forEach((h, i) => {
 fillCell(ws, row, i + 1, h, {
 bold: true, fontSize: 9, color: BRAND.purple700, bgColor: BRAND.purple50,
 ...
 // Summary mode: single merged row
 ws.mergeCells(row, 1, row, 5);
 fillCell(ws, row, 1, `Tổng dịch vụ: ${data.serviceTotal.toLocaleString('vi-VN')} ₫`, {
 bold: true, fontSize: 10, color: BRAND.purple700, bgColor: BRAND.purple50,
 alignment: { horizontal: 'center', vertical: 'middle' },
 });
 ws.getRow(row).height = 24;

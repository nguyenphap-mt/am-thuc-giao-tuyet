...
export async function exportQuotePdf(
 ...
 const menuDisplay = data.menuPriceDisplay || 'detailed';
 ...
 if (menuDisplay === 'summary') {
 // Summary mode: single line, no table
 doc.setFont(FONT_NAME, 'normal');
 doc.setFontSize(9);
 doc.setTextColor(...C.dark);
 doc.text(
 `${data.table_count} bàn × ${formatVND(data.menuTotalPerTable)}/bàn = ${formatVND(data.menuTotalWithTables)}`,
 MARGIN, y + 2
 );
 y += 10;
 } else {
 // detailed or name_only mode
 const showPrice = menuDisplay === 'detailed';
 const menuHead = showPrice
 ? [['STT', 'Tên món', 'Đơn giá/bàn']]
 : [['STT', 'Tên món']];
 const menuBody = data.menuItems.map((item, idx) => showPrice
 ? [String(idx + 1), item.name, formatVND(item.selling_price)]
 : [String(idx + 1), item.name]
 );

 const columnStyles: Record<number, any> = showPrice
 ? { 0: { halign: 'center', cellWidth: 12 }, 1: { halign: 'left' }, 2: { halign: 'right', cellWidth: 35 } }
 : { 0: { halign: 'center', cellWidth: 12 }, 1: { halign: 'left' } };

 const foot = showPrice
 ? [[
 ...
 : [[
 { content: '', styles: { fillColor: C.purple50 } },
 { content: `TỔNG THỰC ĐƠN / BÀN: ${formatVND(data.menuTotalPerTable)}`, styles: { fillColor: C.purple50, fontStyle: 'bold', textColor: C.purple700, halign: 'right' } },
 ]];
 ...
 if (hasServices) {
 const svcDisplay = data.servicePriceDisplay || 'detailed';
 ...
 if (svcDisplay === 'summary') {
 // Summary mode: single line, no table
 doc.setFont(FONT_NAME, 'normal');
 doc.setFontSize(9);
 doc.setTextColor(...C.dark);
 doc.text(`Tổng dịch vụ: ${formatVND(data.serviceTotal)}`, MARGIN, y + 2);
 y += 10;
 } else {
 // Detailed mode: full table with prices

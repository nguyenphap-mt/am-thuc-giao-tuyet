...
export interface QuotePrintData {
 ...
 // Display mode settings (optional, defaults to 'detailed')
 menuPriceDisplay?: 'detailed' | 'name_only' | 'summary';
 servicePriceDisplay?: 'detailed' | 'summary';
}
...
const QuotePrintPreview = forwardRef<HTMLDivElement, QuotePrintPreviewProps>(
 ...
 {(data.menuPriceDisplay || 'detailed') === 'summary' ? (
 <div style={{ ...styles.card, textAlign: 'center' as const }}>
 <p style={{ margin: 0, fontWeight: 600, color: COLOR_PALETTE.purple700, fontSize: '1rem' }}>
 {data.table_count} bàn × {formatCurrency(data.menuTotalPerTable)}/bàn = {formatCurrency(data.menuTotalWithTables)}
 </p>
 </div>
 ) : (
 ...
 {(data.menuPriceDisplay || 'detailed') === 'detailed' && (
 <th style={{ ...styles.th, textAlign: 'right' as const, width: '8rem' }}>Đơn giá/bàn</th>
 )}
 </tr>
 </thead>
 <tbody>
 ...
 <td colSpan={(data.menuPriceDisplay || 'detailed') === 'detailed' ? 2 : 2}
 style={{ padding: '0.5rem 0.75rem', textAlign: 'right' }}>
 Tổng thực đơn / bàn:
 </td>
 {(data.menuPriceDisplay || 'detailed') === 'detailed' ? (
 <td style={{ padding: '0.5rem 0.75rem', textAlign: 'right' as const, fontWeight: 700 }}>
 {formatCurrency(data.menuTotalPerTable)}
 </td>
 ) : (
 <td style={{ padding: '0.5rem 0.75rem', textAlign: 'right' as const, fontWeight: 700 }} colSpan={1}>
 {formatCurrency(data.menuTotalPerTable)}
 </td>
 )}
 </tr>
 ...
 {(data.servicePriceDisplay || 'detailed') === 'summary' ? (
 <div style={{ ...styles.card, textAlign: 'center' as const }}>
 <p style={{ margin: 0, fontWeight: 600, color: COLOR_PALETTE.purple700, fontSize: '1rem' }}>
 ...
 <th style={{ ...styles.th, textAlign: 'right' as const, width: '8rem' }}>Thành tiền</th>
 </tr>
 </thead>
 <tbody>
 ...
 <td style={{ ...styles.td, textAlign: 'right' as const, fontWeight: 500 }}>
 {formatCurrency(service.pricePerUnit * service.quantity)}
 </td>
 </tr>
 ))}
 {data.staffCount > 0 && (
 ...
 <td style={{ ...styles.td, textAlign: 'right' as const, fontWeight: 500 }}>
 {formatCurrency(data.staffCount * data.staffPricePerUnit)}
 </td>
 </tr>
 )}
 </tbody>
